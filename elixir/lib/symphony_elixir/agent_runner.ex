defmodule SymphonyElixir.AgentRunner do
  ...
  require Logger
  alias SymphonyElixir.{AgentRuntime, Config, Linear.Issue, PromptBuilder, Runtime, Tracker, Workspace}
  ...
  defp run_agent_turns(workspace, issue, update_recipient, opts) do
    ...
    with {:ok, runtime_selection} <- AgentRuntime.resolve(issue),
         :ok <- emit_runtime_selection(update_recipient, issue, runtime_selection),
         {:ok, session} <- Runtime.start_session(workspace, runtime_selection) do
      try do
        do_run_agent_turns(session, workspace, issue, update_recipient, opts, issue_state_fetcher, 1, max_turns)
      after
  ...
  defp do_run_agent_turns(app_session, workspace, issue, update_recipient, opts, issue_state_fetcher, turn_number, max_turns) do
    ...
    with {:ok, turn_session} <-
           Runtime.run_turn(
             ...
             prompt,
             issue,
             on_message: runtime_message_handler(update_recipient, issue)
           ) do
  ...
  defp emit_runtime_selection(recipient, issue, selection) when is_map(selection) do
    send_runtime_update(recipient, issue, %{
