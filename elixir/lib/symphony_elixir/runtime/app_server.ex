defmodule SymphonyElixir.Runtime.AppServer do
  @moduledoc """
  Runtime-agnostic client for the app-server JSON-RPC 2.0 stream over stdio.

  Handles session lifecycle (initialize, thread, turn) for any runtime that
  speaks the app-server protocol (Codex, Claude, etc.).
  """

  @behaviour SymphonyElixir.Runtime

  require Logger
  alias SymphonyElixir.{Config, Runtime.DynamicTool}
  ...
  def run_turn(
    ...
    case start_turn(port, thread_id, prompt, issue, workspace, approval_policy, turn_sandbox_policy) do
      {:ok, turn_id} ->
        ...
        Logger.info("Agent session started for #{issue_context(issue)} session_id=#{session_id} runtime=#{effective_runtime}")
        ...
        case await_turn_completion(port, on_message, tool_executor, auto_approve_requests) do
          {:ok, result} ->
            Logger.info("Agent session completed for #{issue_context(issue)} session_id=#{session_id} runtime=#{effective_runtime}")
          ...
          {:error, reason} ->
            Logger.warning("Agent session ended with error for #{issue_context(issue)} session_id=#{session_id} runtime=#{effective_runtime}: #{inspect(reason)}")
      ...
      {:error, reason} ->
        Logger.error("Agent session failed for #{issue_context(issue)} runtime=#{effective_runtime}: #{inspect(reason)}")
  ...
  defp port_metadata(port) when is_port(port) do
    case :erlang.port_info(port, :os_pid) do
      {:os_pid, os_pid} ->
        %{runtime_server_pid: to_string(os_pid), codex_app_server_pid: to_string(os_pid)}
    ...
    case maybe_handle_approval_request(
      ...
      :unhandled ->
        if needs_input?(method, payload) do
          ...
          Logger.debug("Runtime notification: #{inspect(method)}")
          receive_loop(port, on_message, timeout_ms, "", tool_executor, auto_approve_requests)
        end
    end
  end
  ...
  defp log_non_json_stream_line(data, stream_label) do
    ...
    if text != "" do
      if String.match?(text, ~r/\b(error|warn|warning|failed|fatal|panic|exception)\b/i) do
        Logger.warning("Runtime #{stream_label} output: #{text}")
      else
        Logger.debug("Runtime #{stream_label} output: #{text}")
      end
    end
  end
